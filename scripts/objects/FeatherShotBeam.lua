...
function RudeBusterBeam:init(red, x, y, tx, ty, after)
    super.init(self, red and "effects/feathershot/feather" or "effects/feathershot/feather_c", x, y)
...
function RudeBusterBeam:update()
    ...
    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 then
        ...
        for i = 1, 8 do
            local burst = FeatherShotBurst(self.red, self.target_x, self.target_y, math.rad(45 + ((i - 1) * 90)), i > 4)
    ...
    if self.afterimg_timer >= 1 then
        ...
        local sprite = Sprite(self.red and "effects/feathershot/feather" or "effects/feathershot/feather_c", self.x, self.y)
